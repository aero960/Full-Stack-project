...
class CategoryShowEXT extends BuilderComposite implements FastAction
{
    ...
    {
        $sql = "SELECT p.*,category.category_content FROM post_category
            INNER JOIN post p ON pc_post_id=post_id
            INNER JOIN category ON category_id=pc_category_id
    ...
    public function getFastActionResponse()
    {
        $result =$this->getPostToCategory();
        array_walk( $result,function ($index) {
            return [new PostSchema($index), $index->category_content];
        });
...
class AddCategoryEXT extends BuilderComposite implements FastAction
{
    ...
    public function getFastActionResponse()
    {
        ...
        if (!$this->checkValueExist("category", "category_title", $this->title))
            $this->addCategory();
        return ["info" => "Kategoria dodana: " . $this->getCreatedCategory()->category_title];
